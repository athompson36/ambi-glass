...
struct IRDeconvolutionTests {
    
    // Offline test IR deconvolution with known IR
    @Test func testIRDeconvolution() async throws {
        let irkit = IRKit()
        ...
        #expect(sweep.count == Int(sampleRate * seconds), "Sweep length should match")
        
        // Generate inverse
        let inv = irkit.inverseESS(sweep: sweep, sr: sampleRate, seconds: seconds, f0: f0, f1: f1)
        #expect(inv.count == sweep.count, "Inverse length should match sweep")
        ...
        #expect(maxIdx >= 50 && maxIdx <= 150, "Deconvolved peak should be near original IR position")
        #expect(abs(maxVal - 1.0) < 0.5, "Deconvolved peak should be normalized to ~1.0")
    }
}
