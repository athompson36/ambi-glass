...
final class CalibrationKit: ObservableObject {
    ...
    func estimateDelay(reference: [Float], recorded: [Float]) -> Int {
        ...
        // Create conjugate of B: conj(B) = (real, -imag)
        var conjB_imag = [Float](repeating: 0, count: n/2)
        vDSP_vneg(B.imagp, 1, &conjB_imag, 1, vDSP_Length(n/2))
        var conjB = DSPSplitComplex(realp: B.realp, imagp: &conjB_imag)
