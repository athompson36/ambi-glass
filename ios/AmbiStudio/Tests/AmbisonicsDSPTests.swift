...
struct AmbisonicsDSPTests {
    ...
    @Test func testAtoBMapping() async throws {
        ...
        guard let fmt = AVAudioFormat(commonFormat: .pcmFormatFloat32, sampleRate: sampleRate, channels: 4, interleaved: false),
              let aBuf = AVAudioPCMBuffer(pcmFormat: fmt, frameCapacity: AVAudioFrameCount(frameCount)) else {
            throw TestError.bufferCreationFailed
        }
        aBuf.frameLength = AVAudioFrameCount(frameCount)
        
        // Safely access channel data
        guard let channelData = aBuf.floatChannelData else {
            throw TestError.bufferAccessFailed
        }
        
        // Set impulse on channel 0 at sample 100
        channelData[0][100] = 1.0
        channelData[1][100] = 0.0
        channelData[2][100] = 0.0
        channelData[3][100] = 0.0
        ...
        guard let bChannelData = bBuf.floatChannelData else {
            throw TestError.bufferAccessFailed
        }
        let actualW = bChannelData[0][100]
        
        #expect(abs(actualW - expectedW) < 0.001, "W channel should match matrix calculation")
        
        // Test with impulse on channel 1
        channelData[0][100] = 0.0
        channelData[1][100] = 1.0
        let bBuf2 = dsp.processAtoB(aBuffer: aBuf)
        let expectedW2 = matrix[4] * 1.0 // A1 contribution to W
        
        guard let bChannelData2 = bBuf2.floatChannelData else {
            throw TestError.bufferAccessFailed
        }
        let actualW2 = bChannelData2[0][100]
    ...
    @Test func testOrientationTransform() async throws {
        ...
        guard let bChannelData1 = bBuf1.floatChannelData else {
            throw TestError.bufferAccessFailed
        }
        let w1 = bChannelData1[0][100]
        ...
        let x1 = bChannelData1[3][100]
        let y1 = bChannelData1[1][100]
        let x2 = bChannelData2[3][100]
        let y2 = bChannelData2[1][100]
